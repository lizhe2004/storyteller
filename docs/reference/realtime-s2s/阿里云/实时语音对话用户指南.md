...
来源参考 ：https://help.aliyun.com/zh/model-studio/qwen-audio-realtime-user-guides?spm=a2c4g.11186623.help-menu-2400256.d_0_3_10_1.3d29138f7CuwqB
...
该模型除 WebSocket 协议外，还支持通过 AOQ 和 WebRTC 协议接入；如果是客户端对接，且更看重稳定的延迟、弱网下的交互能力、实时双工的降噪与回声消除，可优先考虑 AOQ，协议对比与选型请参见[模型/应用支持力度](/zh/model-studio/realtime-api-overview#rtov-s02h2)。
...
-   支持联网搜索，模型可根据实时问题自主判断是否搜索并返回搜索结果来源（不可与 Function Calling 同时启用）
...
-   语种：德语（de）、英语（en）、西班牙语（es）、法语（fr）、印尼语（id）、意大利语（it）、日语（ja）、韩语（ko）、葡萄牙语（pt）、俄语（ru）、中文（zh）。
-   中文及方言：普通话、广东话、重庆话、东北话、甘肃话、贵州话、浙江话、河北话、河南话、湖北话、湖南话、江西话、宁波话、宁夏话、青岛话、陕西话、山西话、山东话、上海话、四川话、云南话。
...
| **模型** | **音频最大轮次** | **音频最大时长** |
| --- | --- | --- |
| qwen-audio-3.1-realtime-plus | 50  | 300秒 |
| qwen-audio-3.0-realtime-plus | 50  | 300秒 |
| qwen-audio-3.0-realtime-flash | 50  | 300秒 |

音频最大轮次的默认值为 20 轮，最大可上调至 50 轮。调节方式请参见[历史轮次控制](/zh/model-studio/qwen-audio-realtime-user-guides#fc60h313)。

全模态模型的整体选型建议请参见[全模态](/zh/model-studio/omni)。

## 前提条件

-   已[获取与配置 API Key](/zh/model-studio/get-api-key)并将其[配置到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)。
-   如果通过 AOQ 协议接入，需要下载并集成 AOQ 客户端 SDK，详见[AOQ SDK 简介](/zh/model-studio/realtime-api-aoq-sdk-desc)。

## 快速开始

以下基础 WebSocket 示例使用 `qwen-audio-3.1-realtime-plus`，并显式设置系统音色 `longanqian_v3.1`。从 3.0 Plus 迁移时，更新 URL 查询参数 `model`，并按[音色配置](/zh/model-studio/qwen-audio-realtime-user-guides#fc60h311)选择与模型匹配的 `voice`；WebSocket 事件协议保持不变。

通过以下步骤快速体验与 Qwen-Audio 模型的实时语音对话。

#### WebSocket 原生

**说明**各模式下客户端与服务端的 WebSocket 事件交互时序，请参见[事件交互流程](/zh/model-studio/fun-audiochat-realtime-websocket-api#fc63flow01h2)。
...
URL = "wss://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/api-ws/v1/realtime?model=qwen-audio-3.1-realtime-plus"
...
**说明**若 `websockets` 版本低于 11，需将代码中的 `additional_headers` 改为 `extra_headers`，或升级：`pip install --upgrade websockets`。
...
**说明**以上示例使用 server\_vad 模式（服务端声学 VAD 自动检测语音起止）。如需使用 smart\_turn（智能语义轮次）或 push-to-talk（手动控制）模式，请参见[交互模式](/zh/model-studio/qwen-audio-realtime-user-guides#fc60h301)。

## 会话配置

### 交互模式

Qwen-Audio 支持三种交互模式：`server_vad`（声学 VAD 自动检测语音起止）、`smart_turn`（智能语义轮次，声学与语义融合判断）和 push-to-talk（客户端手动控制）。各模式的详细说明及事件交互时序（含时序图），请参见[交互模式](/zh/model-studio/fun-audiochat-realtime-websocket-api#fc63modes01h2)。

**说明**`turn_detection` 仅在首次发送音频之前（IDLE 状态）允许修改。会话建立后切换交互模式需要重新连接。
...
# 以下为华北2（北京）地域的WebSocket URL，调用时请将{WorkspaceId}（含花括号）替换为真实的业务空间ID，各地域的URL不同。
API_URL = "wss://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/api-ws/v1/realtime?model=qwen-audio-3.1-realtime-plus"
...
通过 `voice` 参数设置模型回复的 TTS 音色，3.1 Plus 的默认值为 `longanqian_v3.1`，3.0 Plus/Flash 的默认值为 `longanqian`。支持系统音色和声音复刻音色两种类型。3.1 Plus 保留 3.0 Plus 的音色，并新增 8 个系统音色可选值。

**重要**音色仅可在**第一次** `session.update` 中设置，后续 `session.update` 传入 `voice` 将被忽略。

**系统音色**：根据模型选择音色 ID。

| 模型  | 系统音色 ID |
| --- | --- |
| `qwen-audio-3.1-realtime-plus` | `longanqian`、`longanlingxin`、`longanlingxi`、`longanxiaoxin`、`longanlufeng`、`longanqian_v3.1`、`longanhuan_v3.1`、`longanlingxin_v3.1`、`longanfengyue_v3.1`、`xunanchuan_v3.1`、`beth_v3.1`、`betty_v3.1`、`cally_v3.1` |
| `qwen-audio-3.0-realtime-plus`, `qwen-audio-3.0-realtime-flash` | `longanqian`, `longanlingxin`, `longanlingxi`, `longanxiaoxin`, `longanlufeng` |
...
**声音复刻音色**：通过[声音复刻](/zh/model-studio/voice-cloning-user-guide#qat_vc_h3)方式创建音色（创建时将 `target_model` 设为 `qwen-audio-3.1-realtime-plus`、`qwen-audio-3.0-realtime-plus` 或 `qwen-audio-3.0-realtime-flash`），再将接口返回的 `voice_id` 填入 `voice` 参数。

```
{
    "type": "session.update",
    "session": {
        "voice": "qwen-audio-3.1-realtime-plus-myvoice-xxxxxx"
    }
}
```
...
**说明**一轮响应可包含多个 `function_call`，也可能同时包含普通消息和函数调用。Function Call 部分不会送入 TTS 播报。
...
**说明**若 `conversation.item.create` 指定的 `item.id` 已存在于对话中，会返回错误。
...
-   [WebSocket API](/zh/model-studio/fun-audiochat-realtime-websocket-api)
-   [客户端事件](/zh/model-studio/fun-audiochat-client-events)
-   [服务端事件](/zh/model-studio/qwen-audio-realtime-server-events)
-   [Realtime API 概述](/zh/model-studio/realtime-api-overview)（WebRTC 协议说明）