...
“故事书架”调用 `GET /api/stories`，按创建时间倒序列出项目标题、日期和状态。打开项目调用 `GET /api/stories/{ref}`，可以查看主题、角色、台词和每句音频时长；完整音频通过 `GET /api/stories/{ref}/audio` 播放，单句音频通过 `GET /api/stories/{ref}/segments/{segment}` 获取。

如果服务仍保留生成任务，刷新或重新连接 `/ws?job_id=...` 会收到 `ready`、当前状态、已有剧本预览、角色匹配和剧本就绪事件，然后继续接收队列事件。只知道项目 ID 时，使用故事书架打开已经落盘的结果；不存在完整音频时页面会显示错误，而不会伪造下载链接。
...
“恢复环境变量”会先确认，再调用 reset API 删除该组允许的后台覆盖；之后回到环境变量或程序默认值。设置历史在页面/API 中按最新在前展示，最多保留 100 条。
