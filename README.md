...
| `STORYTELLER_SOUND_DIR` | 全局共享音效库目录，默认 `./sounds` |
...
```bash
# 最简：生成并输出到 outputs/<project_id>.mp3
...
# → outputs/<project_id>.script.json
```
...
通用选项：`--output-dir/-o`、`--project-dir`、`--progress quiet|simple|detailed`、`--strict-mode`、`--log-level`。
...
```
outputs/
├── <project_id>.mp3              # 拼接后的最终音频
├── <project_id>.script.json      # 完整剧本（含角色、台词、演法指令、匹配到的音色）
└── <project_id>/audio/           # 逐句合成的分段音频（用于断点续作）
```
...
所有项目共用一个音效目录（默认 `./sounds/`），每条声音以「模型 + 归一化提示词 + 格式」的哈希为指纹登记在 `sounds/index.json`：
...
项目元数据保存在 `projects/<project_id>/project.json`，状态机：
