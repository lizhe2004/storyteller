...
export const api = {
  ...
  voices: (filters: { provider?: string; model?: string; gender?: string; age?: string; status?: string; page?: number; page_size?: number } = {}) => {
  ...
  updateVoiceEnabled: (key: string, enabled: boolean) => request<any>('/api/voices/' + encodeURIComponent(key), {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({enabled})}),
