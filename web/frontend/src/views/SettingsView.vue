...
function providerPatch(group: string) {
  ...
  const patch: any = { providers: String(value.providers_text || '').split(',').map((item: string) => item.trim()).filter(Boolean), default_provider: value.default_provider, provider_config: {} }
  for (const [name, config] of Object.entries(value.provider_config || {})) {
    const item: any = {}
    for (const key of ['type', 'model', 'endpoint', 'base_url', 'resource_id']) if (config && (config as any)[key] !== undefined) item[key] = (config as any)[key]
...
async function resetGroup(group: string) {
  ...
  for (const key of Object.keys(value || {})) if (key !== 'provider_config' && key !== 'host' && key !== 'port') paths.push(`${group}.${key}`)
  for (const name of Object.keys(value?.provider_config || {})) for (const key of ['type', 'api_key', 'model', 'endpoint', 'base_url', 'resource_id']) paths.push(`${group}.provider_config.${name}.${key}`)
