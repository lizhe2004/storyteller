  ...
  <section class="voice-management-page">
    ...
    <div v-else class="voice-table">
      <article v-for="voice in voices" :key="voice.key" data-testid="voice-row" class="voice-row">
        <div class="voice-main"><strong>{{ voice.name || voice.voice_id }}</strong><span>{{ voice.provider }} · {{ voice.model || '默认模型' }}</span><small>{{ voice.voice_id }} · {{ labelGender(voice.gender) }}</small></div>
